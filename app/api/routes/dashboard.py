...
async def get_latest_data(user: CurrentUser):
    """Obtiene los últimos datos de sensores filtrados por usuario."""
    try:
        return await supabase_db.get_latest_sensors(user["id"])
...
async def get_history_data(user: CurrentUser):
    """Obtiene el historial reciente filtrado por usuario."""
    try:
        return await supabase_db.get_sensor_history_raw(user["id"])
    except Exception as e:
